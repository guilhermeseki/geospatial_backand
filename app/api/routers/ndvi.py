# app/api/routers/ndvi.py
"""
NDVI API endpoints (Sentinel-2, MODIS)
...
def _query_point_data_sync(historical_ds: xr.Dataset, request: NDVIHistoryRequest | NDVITriggerRequest, is_trigger: bool):
    """
    ...
    ts_values = ts["ndvi"].compute().squeeze()

    data_vals = np.atleast_1d(ts_values.values)
    time_vals = np.atleast_1d(ts_values.time.values)

    if is_trigger:
        # Trigger logic - support both "below" and "above" thresholds
        trigger_type = getattr(request, 'trigger_type', 'below')
        ...
        exceeded_values = np.round(np.array(data_vals[exceedances], dtype=float), 3)
        ...
        # History logic
        history = {
            str(pd.Timestamp(t).date()): (round(float(v), 3) if not np.isnan(v) else None)
...
async def get_ndvi_history(request: NDVIHistoryRequest):
    """Returns historical NDVI values for a given lat/lon and date range."""
    source = request.source
    ...
    historical_ds = get_dataset('ndvi', source)

    if historical_ds is None:
        raise HTTPException(
            status_code=503,
            detail=f"Historical NDVI data for source '{source}' is not yet loaded or is unavailable."
        )
    ...
    try:
        ...
        logger.error(f"Error querying NDVI history data for {source}: {e}")
        logger.error(traceback.format_exc())
        raise HTTPException(status_code=500, detail="Error querying historical NDVI data")


@router.post("/triggers")
@retry_on_failure(max_retries=2, exceptions=(RuntimeError, ValueError, KeyError))
async def get_triggers(request: NDVITriggerRequest):
    ...
    try:
        ...
        logger.error(f"Error querying NDVI triggers data for {source}: {e}")
        logger.error(traceback.format_exc())
        raise HTTPException(status_code=500, detail="Error querying historical NDVI data")


# --- SYNCHRONOUS HELPER FOR AREA QUERIES ---
def _calculate_area_exceedances_sync(historical_ds: xr.Dataset, request: NDVITriggerAreaRequest):
    """
    ...
    # Trigger logic - support both "below" and "above"
    trigger_type = getattr(request, 'trigger_type', 'below')
    if trigger_type == "below":
        trigger_mask_3D = (ndvi_data < request.trigger) & circular_mask
    else:
        trigger_mask_3D = (ndvi_data > request.trigger) & circular_mask

    # Extract values and stack
    exceeding_values = ndvi_data.where(trigger_mask_3D)
...
@retry_on_failure(max_retries=2, exceptions=(RuntimeError, ValueError, KeyError))
async def get_area_triggers(request: NDVITriggerAreaRequest):
    """Area-based trigger calculation for NDVI."""
    source = request.source
    ...
    try:
        ...
        logger.error(f"Error processing NDVI area triggers for {source}: {e}")
        logger.error(traceback.format_exc())
...
async def get_ndvi_featureinfo(request: NDVIRequest):
    """Returns the NDVI pixel value for a given lat/lon from the raster."""
    if request.lat is None or request.lon is None:
        raise HTTPException(status_code=400, detail="lat and lon are required")

    # Map source names to GeoServer layer names
    source_to_layer = {
        'sentinel2': 'ndvi_s2',
        'modis': 'ndvi_modis'
    }

    layer_suffix = source_to_layer.get(request.source)
    if not layer_suffix:
    ...
    try:
        ...
        if features and len(features) > 0:
            ...
            if pixel_value is not None:
                pixel_value = round(float(pixel_value), 3)
        else:
            pixel_value = None
        ...
        logger.warning(f"HTTP error fetching NDVI feature info: {e}")
        raise
        ...
        logger.error(f"Unexpected error in NDVI featureinfo: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Error processing feature info request")


@router.post("/polygon")
async def ndvi_polygon(request: PolygonRequest):
    """
    Process polygon request for NDVI data.
    ...
    # Validate source
    if source not in ['sentinel2', 'modis']:
        raise HTTPException(
            status_code=400,
            detail=f"Invalid source for NDVI. Must be 'sentinel2' or 'modis'"
        )

    # Get dataset
    ds = get_dataset('ndvi', source)

    if ds is None:
        raise HTTPException(
            status_code=503,
            detail=f"NDVI data for source '{source}' is not loaded"
        )

    try:
        # Process in thread pool
        result = await asyncio.to_thread(
            _process_ndvi_polygon_sync,
            ds,
            request
        )
        ...
        logger.error(f"Error processing NDVI polygon request: {e}")
        logger.error(traceback.format_exc())
        raise HTTPException(status_code=500, detail=str(e))


def _process_ndvi_polygon_sync(ds, request: PolygonRequest):
    """Synchronous helper for NDVI polygon processing."""
    # Create polygon
    ...
    result = PolygonProcessor.process_polygon_request(
        ds=ds,
        polygon=polygon,
        variable_name="ndvi",  # NDVI variable name
        start_date=request.start_date,
        end_date=request.end_date,
