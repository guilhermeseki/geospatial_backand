"""
MERGE Full Backfill Script
Checks ALL dates from 2014-11-01 to yesterday and fills any gaps.
...
if __name__ == "__main__":
    logger.info("=" * 80)
    logger.info("MERGE FULL BACKFILL - ONE-TIME OPERATION")
    logger.info("=" * 80)

    # Check current state
    mosaic_dir = Path(settings.DATA_DIR) / "merge"
    existing_files = list(mosaic_dir.glob("merge_*.tif"))
    ...
    # Full backfill range (MERGE starts from Nov 2014)
    ...
    # Final count
    final_files = list(mosaic_dir.glob("merge_*.tif"))
