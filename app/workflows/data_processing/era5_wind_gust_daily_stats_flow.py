"""
ERA5 Daily Wind Gust Flow (Pre-computed Statistics)
Downloads DAILY MAXIMUM wind gust from ERA5 daily statistics.
Dataset: derived-era5-single-levels-daily-statistics
Variable: instantaneous_10m_wind_gust with daily_statistic=daily_maximum

This is MORE EFFICIENT than downloading 24 hourly values!
"""
...
def download_era5_daily_max_wind_gust(
    start_date: date,
    end_date: date,
    area: List[float]  # [N, W, S, E]
) -> Path:
    """
    Download DAILY MAXIMUM 10m wind gust from ERA5 daily statistics.
    ...
    # Prepare output path
    raw_dir = Path(settings.DATA_DIR) / "raw" / "era5_wind_gust_daily"
    raw_dir.mkdir(parents=True, exist_ok=True)

    start_str = start_date.strftime("%Y%m%d")
    end_str = end_date.strftime("%Y%m%d")
    output_path = raw_dir / f"wind_gust_daily_max_{start_str}_{end_str}.nc"

    if output_path.exists():
        logger.info(f"Daily max wind gust already downloaded: {output_path}")
        return output_path
    ...
    request = {
        ...
        "time_zone": "utc+00:00",
        "frequency": "1_hourly",  # Compute from all 24 hours
        "area": area,  # [N, W, S, E]
        "format": "netcdf"
    }

    logger.info(f"Downloading daily MAX wind gust: {start_date} to {end_date}")
    logger.info(f"Dataset: derived-era5-single-levels-daily-statistics")
    logger.info(f"Variable: 10m_wind_gust_since_previous_post_processing")
    logger.info(f"  (This is MAXIMUM gust, not instantaneous!)")
    logger.info(f"Daily statistic: MAXIMUM (pre-computed)")
    logger.info(f"Frequency: 1_hourly (uses all 24 hours)")
    ...
    try:
        ...
        result = client.retrieve("derived-era5-single-levels-daily-statistics", request)
...
@task
def process_daily_max_wind_gust_to_geotiff(
    netcdf_path: Path,
    dates_to_process: Optional[List[date]] = None
) -> List[Path]:
    """
    Process daily maximum wind gust NetCDF to GeoTIFFs.

    Args:
        netcdf_path: Path to daily max wind gust NetCDF
    ...
    try:
        ...
        var_name = None
        for possible_name in ['10fg', 'fg10', '10m_wind_gust_since_previous_post_processing',
                              'wind_gust', '10m_wind_gust', 'i10fg', 'instantaneous_10m_wind_gust']:
        ...
        for possible_time in ['valid_time', 'time']:
            if possible_time in gust_data.dims:
                time_dim = possible_time
                break

        if not time_dim:
            raise ValueError(f"Time dimension not found. Available dims: {list(gust_data.dims)}")

        logger.info(f"Using time dimension: {time_dim}")
        logger.info(f"Processing {len(gust_data[time_dim])} days")

        # Process each day
        for time_val in gust_data[time_dim].values:
            ...
            # Get daily max for this date (already computed by CDS!)
            daily_gust = gust_data.sel({time_dim: time_val})

            # Convert from m/s to km/h
            daily_gust = daily_gust * 3.6

            # Set CRS and mark NoData values
            daily_gust = daily_gust.rio.write_crs("EPSG:4326")
            daily_gust = daily_gust.rio.write_nodata(np.nan)
...
@flow(name="process-era5-wind-gust-daily-stats")
def era5_wind_gust_daily_stats_flow(
    start_date: date,
    end_date: date,
    skip_existing: bool = True
) -> List[Path]:
    """
    Download DAILY MAXIMUM wind gust from ERA5 daily statistics and create GeoTIFFs.

    This uses pre-computed daily statistics - MUCH more efficient than hourly!
    ...
    logger.info("="*80)
    logger.info("ERA5 DAILY MAX WIND GUST (PRE-COMPUTED STATISTICS)")
    logger.info("="*80)
    logger.info(f"Period: {start_date} to {end_date}")
    logger.info(f"Source: ERA5 daily statistics (derived-era5-single-levels-daily-statistics)")
    logger.info(f"Variable: 10m_wind_gust_since_previous_post_processing")
    logger.info(f"  (MAXIMUM gust over each hour, not instantaneous snapshot)")
    logger.info(f"Statistic: Daily maximum (pre-computed from 24 hourly maxima)")
    logger.info("="*80)
    ...
    # Download daily max wind gust data (Brazil bbox)
    brazil_bbox = [6.55, -75.05, -35.05, -33.45]  # [N, W, S, E]
    netcdf_path = download_era5_daily_max_wind_gust(
    ...
    # Process to GeoTIFFs
    processed = process_daily_max_wind_gust_to_geotiff(
