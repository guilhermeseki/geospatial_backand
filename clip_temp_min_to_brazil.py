#!/usr/bin/env python3
"""
Clip all temp_min GeoTIFF files to Brazil shapefile boundaries.
...
# Paths
DATA_DIR = Path("/mnt/workwork/geoserver_data/temp_min")
SHAPEFILE = Path("/opt/geospatial_backend/data/shapefiles/br_shp/brazil_b10km.shp")
BACKUP_DIR = Path("/mnt/workwork/geoserver_data/temp_min_backup")
...
def clip_geotiff(input_file: Path, shapefile: Path) -> bool:
    ...
    try:
        # Run gdalwarp with shapefile cutline
        result = subprocess.run([
            "gdalwarp",
            "-q",  # Quiet mode
            "-cutline", str(shapefile),
            "-crop_to_cutline",  # Crop to exact cutline extent
            "-dstnodata", "nan",
            "-co", "COMPRESS=LZW",
            "-co", "TILED=YES",
...
def main():
    ...
    # Find all temp_min GeoTIFF files
    geotiff_files = sorted(DATA_DIR.glob("temp_min_*.tif"))

    if not geotiff_files:
        logger.warning(f"No temp_min_*.tif files found in {DATA_DIR}")
        return

    logger.info(f"Found {len(geotiff_files)} temp_min GeoTIFF files to clip")
    logger.info(f"Using shapefile: {SHAPEFILE}")

    # Log operation info
    logger.info("")
    logger.info("="*70)
    logger.info(f"CLIPPING {len(geotiff_files)} TEMP_MIN FILES TO BRAZIL BOUNDARIES")
    logger.info("="*70)
