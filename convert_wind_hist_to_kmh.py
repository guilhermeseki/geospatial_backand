#!/usr/bin/env python3
"""
Convert all wind_speed historical NetCDF files from m/s to km/h.
...
def main():
    ...
    logger.info("=" * 80)
    logger.info("CONVERTING WIND SPEED HISTORICAL NetCDF FROM m/s TO km/h")
    logger.info("=" * 80)
    logger.info(f"Directory: {hist_dir}")
    logger.info(f"Files to convert: {len(nc_files)}")
    ...
    for nc_file in nc_files:
        try:
            ...
            # Check current units
            current_units = ds['wind_speed'].attrs.get('units', 'unknown')
            logger.info(f"  Current units: {current_units}")
            ...
            import tempfile
            import shutil
            temp_dir = Path(tempfile.mkdtemp(prefix="wind_hist_convert_"))
            ...
            logger.error(f"  ✗ Failed to convert {nc_file.name}: {e}")
            import traceback
