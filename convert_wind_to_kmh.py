#!/usr/bin/env python3
"""
Convert all wind_speed GeoTIFF files from m/s to km/h.
Multiplies all values by 3.6 (1 m/s = 3.6 km/h)
"""
...
def convert_wind_file_to_kmh(tif_file: Path) -> bool:
    """Convert a single wind speed GeoTIFF from m/s to km/h."""
    try:
        # Read the file
        ...
        if nodata is not None:
            mask = data == nodata
            data_kmh[mask] = nodata

        # Write back to the same file
        with rasterio.open(tif_file, 'w', **profile) as dst:
            dst.write(data_kmh.astype(rasterio.float32), 1)

        return True

    except Exception as e:
        logger.error(f"Failed to convert {tif_file.name}: {e}")
        return False
...
def main():
    ...
    logger.info("=" * 80)
    logger.info("CONVERTING WIND SPEED FROM m/s TO km/h")
    logger.info("=" * 80)
    logger.info(f"Directory: {wind_dir}")
    logger.info(f"Files to convert: {len(tif_files)}")
    logger.info(f"Conversion factor: 3.6 (1 m/s = 3.6 km/h)")
    logger.info("=" * 80)
    ...
    for tif_file in tqdm(tif_files, desc="Converting files"):
        if convert_wind_file_to_kmh(tif_file):
            success_count += 1
        else:
            fail_count += 1
    ...
    if tif_files:
        sample_file = tif_files[len(tif_files)//2]  # Pick middle file
        logger.info(f"\nVerifying conversion with {sample_file.name}...")
