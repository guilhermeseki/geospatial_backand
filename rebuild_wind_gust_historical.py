...
Source: reanalysis-era5-single-levels (FULL ERA5 - has wind gust!)
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('/tmp/rebuild_wind_gust.log'),
...
def main():
    ...
    logger.info("="*80)
    logger.info("REBUILDING WIND GUST HISTORICAL DATA")
    logger.info("="*80)
    logger.info("Source: ERA5 reanalysis (reanalysis-era5-single-levels)")
    logger.info("Variable: instantaneous_10m_wind_gust")
    ...
    if failed_years:
        ...
        logger.info("\nNext steps:")
        logger.info("1. Verify a few dates with test_era5_wind_gust.py")
        logger.info("2. Build historical NetCDF (if needed)")
        logger.info("3. Restart the API to load new data")
        return 0
